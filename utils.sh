...
echo "1) Copy .desktop file"
echo "2) Remove .desktop file"
echo "3) Exit"
read -p "Enter the number of your choice: " action

case $action in
  1)
    # Copy .desktop file action
    echo "Select .desktop file to copy:"
    select selected_file in $(find "$PREFIX/var/lib/proot-distro/installed-rootfs/debian/usr/share/applications" -type f -name "*.desktop"); do
  ...
  2)
    # Remove .desktop file action
    echo "Select .desktop file to remove:"
    select selected_file in $(find "$PREFIX/share/applications" -type f -name "*.desktop"); do
      if [ -z "$selected_file" ]; then
        echo "Invalid selection. Exiting..."
        exit 1
      fi
      ...
      echo "File '$desktop_filename' has been removed successfully!"
      break
    done
    ;;
